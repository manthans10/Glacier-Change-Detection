...
imageCollection = ['Dec 1984.jpg', 'Dec 1988.jpg', 'Dec 1992.jpg',
                   'Dec 1996.jpg', 'Dec 2000.jpg', 'Dec 2004.jpg', 'Dec 2008.jpg', 'Dec 2012.jpg', 'Dec 2016.jpg', 'Dec 2019.jpg']
...
while True:
    ...
    for i in imageCollection:
        ...
        lower = np.array([0,0,255])  # h_min,s_min,v_min
        upper = np.array([255,255,255])  # h_max,s_max,v_max
        mask = cv2.inRange(imgHsv, lower, upper)
        result = cv2.bitwise_and(img, img, mask=mask)

        maskImgShape = img.shape
        # print(maskImgShape[0], maskImgShape[1])
        totalArea = int((maskImgShape[0] * maskImgShape[1]) * 27600.1)/1000000
        whitepart = np.sum(mask >= 100)
        whitepart = int(whitepart*27600.1)/1000000
        areaSqkm.append(whitepart)
        yearData.append(imageName)
        whiteAreaArray.append(str(imageName) + ' =   ' +
                              str(whitepart) + ' ' + 'sq km')
        ...
        resized_original_text = cv2.putText(img=resized_original, text=str(imageTitle), org=(
            10, 20), fontFace=cv2.FONT_HERSHEY_DUPLEX, fontScale=2.0, color=(125, 246, 55), thickness=5)
    ...
    print(whiteAreaArray)
    whiteAreaArray.clear()
